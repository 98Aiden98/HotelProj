...
>({
  ...
  useEffect(() => {
    if (redirectNeeded) {
      navigate(getAllMemoriesRoute(), { replace: true });
    }
  }, [redirectNeeded, navigate]);

  if (
    queryResult?.isLoading ||
    (queryResult?.isFetching && showLoaderOnFetching) ||
    redirectNeeded
  ) {
...
>(
  ...
  const hoc = (PageComponent: React.FC<TProps>) => {
    const WrappedComponent: React.FC = () => (
      <PageWrapper {...pageWrapperProps} Page={PageComponent} />
    );
    const name = PageComponent.displayName || PageComponent.name || "Component";
    WrappedComponent.displayName = `WithPageWrapper(${name})`;
    return WrappedComponent;
  };
  hoc.displayName = "withPageWrapper";
  return hoc;
};
